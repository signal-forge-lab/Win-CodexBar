...
- In API mode CodexBar also reads the current AIHubMix Transactions feed (`/call/usr/quota_rec`) and uses the newest active positive grant's `balance_after` as the funded-balance anchor. The console itself labels this field “balance after”. A current $7 balance whose latest funding event left $10 is therefore 30% used / 70% remaining in FloatBar. Failed, expired, deduction, malformed, or stale-smaller records are ignored. If transaction enrichment is unavailable, the provider safely falls back to balance-only display.
- The console Transactions route uses user-scoped auth rather than a Manage Key directly on all deployments. CodexBar first tries the Manage Key with the explicit user id; when necessary it reuses the `access_token` already returned by the same authenticated `GET /api/user/self` response. That access token is memory-only and is never persisted, rendered, or logged. CodexBar deliberately does **not** call `GET /api/user/token` for enrichment because compatible backends may implement that endpoint as access-token regeneration rather than a read-only lookup.
...
- `Auto` uses only the configured Manage Key and never initiates a DevTools/CDP connection, including when the API request fails. This prevents periodic provider refreshes from repeatedly triggering Edge remote-debugging permission prompts.
