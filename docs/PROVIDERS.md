...
- The bridge refreshes only already-open AI Studio Spend tabs on the same three-minute alarm as Gemini Apps and never creates a Spend tab on its own. If no Spend tab is open, the provider can still use its sanitized last-known-good cache and, on Windows, its local CDP fallback when an approved DevTools endpoint is available.
...
- Optional API auth uses an AIHubMix **Manage Key / system access token** from Settings → Providers or the official CLI-compatible `AIHUBMIX_TOKEN` environment variable (`AIHUBMIX_MANAGE_KEY` / `AIHUBMIX_ACCESS_TOKEN` are also accepted aliases). This credential is distinct from a normal model API key. The documented `https://api.aihubmix.com` backup domain is used only when the primary platform endpoint fails for a non-auth reason.
- `Auto` prefers an explicitly configured Manage Key. If it is missing or unavailable, Windows falls back to an already-approved local Chromium DevTools endpoint and opens the Top Up page in a hidden/background target. CodexBar reads only the authenticated `/api/user/self` response body and emits sanitized numeric `quota` / `used_quota` values; it never extracts cookies, localStorage, Authorization headers, access tokens, email, or raw browser state.
- `Web` forces the signed-in browser/CDP balance path. The provider is disabled by default; enable it after either signing in to `console.aihubmix.com` in the approved Edge/Chrome profile or configuring a Manage Key.
